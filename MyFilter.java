...
public class MyFilter {
    ...
    /**
     * A function that filters 'Client' and 'Fournisseur' by 'Ville'.
     * @param model original model to be changed
     * @param ville filter
     * @return filtered model
     */
    public static DefaultTableModel FilterComptesByVille(DefaultTableModel model, Ville ville){
        model.setRowCount(0);
                
        if(ville !=null && !Globals.map_villes.isEmpty()){
                    
            Iterator it = Globals.map_clients.entrySet().iterator();
            while(it.hasNext()){
                Map.Entry<String,Client> pair = (Map.Entry)it.next();
                if(pair.getValue().getVilleCompte().getnoVille()== ville.getnoVille()){
                    Object[]data ={
                        pair.getValue().getNoCompte()+"",
                        "Client",
                        pair.getValue().getNomCompte()+"",
                        pair.getValue().getSolde()+""
                    };
        ...
        if(ville != null && !Globals.map_villes.isEmpty()){            
            ...
            while(it.hasNext()){
                Map.Entry<String,Fournisseur> pair = (Map.Entry)it.next();
                if(pair.getValue().getVilleCompte().getnoVille() == ville.getnoVille()){
                    Object[]data ={
                        pair.getValue().getNoCompte()+"",
                        "Fournisseur",
