...
public class MyFunctions{
    ...
    public static void AddSortedDataVilles(){
        //sort again
        ArrayList<Ville> arr_sort = new ArrayList();
        Iterator it = Globals.map_villes.entrySet().iterator();
        while(it.hasNext()){
            Map.Entry<Integer,Ville> pair = (Map.Entry<Integer,Ville>)it.next();
            arr_sort.add(pair.getValue());
        }
        Collections.sort(arr_sort, Comparators.VilleComparator());
        Globals.map_villes = new LinkedHashMap();
        it = arr_sort.iterator();
        while(it.hasNext()){
            Ville v = (Ville) it.next();
            Globals.map_villes.put(v.getnoVille(), v);
        }
        MyFunctions.AddData(Globals.FileName.Villes, Globals.map_villes);
    }
